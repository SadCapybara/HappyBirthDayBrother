...
const wishes = [
  ...
  {
    label: "🦝",
    wish: "🦝Енот украл это поздравление🦝 Желаю тебе найти украденное Енотом поздравление! (он где то рядом)",
    weight: 0.05
  }
];
...
textureImg.onload = () => {
  ...
  soundToPlay.play();
        showResultFixed(selectedIndex,stringAdded);
  ...
  function showResultFixed(index, stringAdded="") {
  const overlay = document.getElementById("overlay");
  const wishText = document.getElementById("wish-text");
  const spinAgainButton = document.getElementById("spin-again");

  wishText.textContent = stringAdded+wishes[index].wish;
